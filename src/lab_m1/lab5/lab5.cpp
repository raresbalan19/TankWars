...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-3, 0.5f, 2));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_I)) orthoTop += deltaTime; 
    if (window->KeyHold(GLFW_KEY_K)) orthoBottom -= deltaTime; 
    if (window->KeyHold(GLFW_KEY_J)) orthoLeft -= deltaTime; 
    if (window->KeyHold(GLFW_KEY_L)) orthoRight += deltaTime;  

    // Actualizăm matricea de proiecție ortografică dacă nu suntem în perspectivă
    if (!isPerspective) {
